...
def update_task(args):
    ...
    for t in tasks:
        if str(t["id"]) == tid:
            ...
            # Only append to notes when --notes is provided
...
def append_task(args):
    ...
    for t in tasks:
        if str(t["id"]) == tid:
            if args.AD:
                # Append to description, initializing if necessary
                ...
                return
            if args.AN:
                # Append to notes, initializing if necessary
                if t.get("notes"):
        ...
        for sub in t.get("subtasks", []):
            if sub["id"] == tid:
                if args.AD:
                    # Append to subtask description, initializing if necessary
                    ...
                    return
                if args.AN:
                    # Append to subtask notes, initializing if necessary
                    if sub.get("notes"):
